...
\begin{theorem}
    Let $V=C(\mathbb{R}/\mathbb{Z};\mathbb{C})$ be a complex-valued, $1$-periodic, continuous function space over $\mathbb{C}$. 
    $V$ over $\mathbb{C}$ is a vector space. 
\end{theorem}
...
Then $(V,\langle-,-\rangle)$ is an inner product space.
\end{theorem}

\begin{pfbox}
    Take $f,g,h\in V$ and $\alpha,\beta\in\mathbb{C}$.
    
    (1) Linear in 1st component:
    \begin{align*}
        \langle\alpha f+\beta g,h\rangle &= \int_{0}^{1}(\alpha f(x)+\beta g(x))\overline{h(x)}dx  \\
        &= \alpha\int_{0}^{1}f(x)\overline{h(x)}dx + \beta\int_{0}^{1}g(x)\overline{h(x)}dx  \\
    ...
    \begin{align*}
        ...
        &= \overline{\int_{0}^{1}g(x)\overline{f(x)}dx} = \overline{\langle g,f\rangle}.
    \end{align*}

    (3) Non-Negativity:
    $$ \langle f,f\rangle=\int_{0}^{1}f(x)\overline{f(x)}dx=\int_{0}^{1}|f(x)|^{2}dx\ge0.$$

    (4) Non-Degeneracy:
    Suppose $\langle f,f\rangle=0$. By (3), $|f(x)|^{2}\ge0$ and $\int_{0}^{1}|f(x)|^{2}dx=0$. 
    This forces $|f(x)|^{2}=0 \Rightarrow Re(f(x))=0, Im(f(x))=0 \Rightarrow f(x)\equiv0$.
\end{pfbox}

\begin{theorem}
    Let $(V,\langle-,-\rangle)$ be a complex inner product space, for $f,g,h\in V$ and $\alpha,\beta\in\mathbb{C}$.
    \begin{enumerate}
        \item Almost linear in 2nd comp: $\langle f,\alpha g+\beta h\rangle=\overline{\alpha}\langle f,g\rangle+\overline{\beta}\langle f,h\rangle$.
        \item $||\alpha f||=|\alpha|||f||$.
        \item $\langle f,g\rangle=0$ for any $g\in V \Leftrightarrow f\equiv0$.
        \item $\langle f,h\rangle=\langle g,h\rangle$ for any $h\in V \Leftrightarrow f=g$.
    \end{enumerate}
\end{theorem}

\begin{pfbox}
    Once we prove the general verison, the above theorem is a direct result.
\end{pfbox}

\begin{theorem}
    Let $(V,\langle-,-\rangle)$ be a complex inner product space, for $f,g\in V$.
    \begin{itemize}
        \item Cauchy-Schwarz: $|\langle f,g\rangle| \le ||f||\cdot||g||$.
        \item Triangle: $||f+g||\le||f||+||g||$.
    \end{itemize}
\end{theorem}

\begin{pfbox}
    We provide an alternative proof which is specific to this inner product space.
    
    \textbf{Proof of Cauchy-Schwarz}:
    Let $u,v\in C(\mathbb{R}/\mathbb{Z};\mathbb{R})$. We first prove the real-valued function case.
    For any $\lambda\in\mathbb{R}$, consider $z(x)=u(x)-\lambda v(x)$.
    If $v(x)\equiv0$, then it is trivial. 
    Now suppose $v(x)\not\equiv0$.
    By non-negativity, $|z(x)|^{2}\ge0 \Rightarrow \langle z(x),z(x)\rangle=\int_{0}^{1}(u(x)-\lambda v(x))^{2}dx\ge0$.
    $$\Rightarrow \int_{0}^{1}u^{2}(x)dx-2\lambda\int_{0}^{1}u(x)v(x)dx+\lambda^{2}\int_{0}^{1}v^{2}(x)dx\ge0.$$
    Set $f(\lambda)=A\lambda^{2}-2B\lambda+C$, where $A=\int_{0}^{1}v^{2}(x)dx$, $B=\int_{0}^{1}u(x)v(x)dx$, $C=\int_{0}^{1}u^{2}(x)dx$.
    To find the minimum, set $f'(\lambda)=0$, which gives $\lambda^{*}=\frac{B}{A}=\frac{\int_{0}^{1}u(x)v(x)dx}{\int_{0}^{1}v^{2}(x)dx}$.
    Since $f(\lambda)\ge0$ for all $\lambda$, we have $f\left(\frac{B}{A}\right)=\frac{-B^{2}+AC}{A}\ge0$.
    $$\Rightarrow B^{2}\le AC \Rightarrow B\le A^{\frac{1}{2}}C^{\frac{1}{2}}.$$
    $$\Rightarrow \int_{0}^{1}u(x)v(x)dx\le\left(\int_{0}^{1}u^{2}(x)dx\right)^{\frac{1}{2}}\left(\int_{0}^{1}v^{2}(x)dx\right)^{\frac{1}{2}}.$$
    
    \textbf{Proposition}: $\left|\int_{0}^{1}f(x)dx\right|\le\int_{0}^{1}|f(x)|dx$ for all $f\in C(\mathbb{R}/\mathbb{Z};\mathbb{C})$.
    Let $I=\int_{0}^{1}f(x)dx$. If $I=0$ it is trivial. Suppose $I\ne0, I\in\mathbb{C}$.
    Consider $c=\frac{\overline{I}}{|I|}$, so $|c|=1$.
    Note that $cf(x)=Re(cf(x))+i Im(cf(x))$ and $|cf(x)|=\sqrt{Re(cf(x))^{2}+Im(cf(x))^{2}}\ge Re(cf(x))$.
    Then $cI=c\int_{0}^{1}f(x)dx=\int_{0}^{1}Re(cf(x))dx+i\int_{0}^{1}Im(cf(x))dx=|I|$.
    Since $|I|$ is real, the imaginary part is $0$, thus:
    $$|I|=\left|\int_{0}^{1}f(x)dx\right|=\int_{0}^{1}Re(cf(x))dx\le\int_{0}^{1}|cf(x)|dx=\int_{0}^{1}|c||f(x)|dx=\int_{0}^{1}|f(x)|dx.$$
    
    Now, for complex-valued functions $f,g$:
    $$|\langle f,g\rangle|=\left|\int_{0}^{1}f(x)\overline{g(x)}dx\right|\le\int_{0}^{1}\left|f(x)\overline{g(x)}\right|dx=\int_{0}^{1}|f(x)||g(x)|dx.$$
    Using the real-valued Cauchy-Schwarz inequality on $|f(x)|$ and $|g(x)|$:
    $$\le\left(\int_{0}^{1}|f(x)|^{2}dx\right)^{\frac{1}{2}}\left(\int_{0}^{1}|g(x)|^{2}dx\right)^{\frac{1}{2}}=||f||\cdot||g||.$$
    ...
    \begin{align*}
        ...
        &= \langle f,f\rangle+2Re(\langle f,g\rangle)+\langle g,g\rangle \\
        &\le ||f||^{2}+2|\langle f,g\rangle|+||g||^{2} \\
        &\le ||f||^{2}+2||f||||g||+||g||^{2} = (||f||+||g||)^{2}.
    \end{align*}
    Hence $||f+g||\le||f||+||g||$.
\end{pfbox}
...
\begin{theorem}
    Let $f,g\in C(\mathbb{R}/\mathbb{Z}; \mathbb{C})$:
    \begin{enumerate}
        \item (Parallelogram identity) $\|f+g\|_{2} + \|f-g\|_{2} = 2(\|f\|_{2} + \|g\|_{2}).$
...
\begin{remark}
    Endowed with $L^2$ norm, two functions are orthogonal if $\langle f,g\rangle=0$.
\end{remark}
...
\begin{theorem}
 $(C(\mathbb{R}/\mathbb{Z}; \mathbb{C}), d_\infty)$ is a complete metric space.
\end{theorem}
...
\begin{theorem}
    The set $\{e_{n}\}_{n\in\mathbb{Z}}$ is orthonormal.
\end{theorem}

\begin{pfbox}
    By direct integration,
    $$ \langle e_{n},e_{m}\rangle=\int_{0}^{1}e^{2\pi i(n-m)x}dx = \delta_{n,m} = \begin{cases} 1 & \text{if } n=m \\ 0 & \text{if } n\ne m \end{cases}.$$
\end{pfbox}

\begin{remark}
    We cannot answer if the set is exactly an ONB right now.
\end{remark}
...
\begin{enumerate}
    ...
    \item (Bilinearity) 
    \begin{enumerate}
        \item Fix $h, \,(f+g)*h = f*h + g*h$
        \item Fix $f, \, f*(g+h)=f*g + f*h$
        \item $c(f*g)=(cf)*g=f*(cg).$
    \end{enumerate}
\end{enumerate}    
\end{property}

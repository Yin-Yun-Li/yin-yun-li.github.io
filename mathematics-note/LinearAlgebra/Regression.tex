...
\begin{theorem}[Gram-Schmidt Process]
    ...
    \begin{enumerate}
        ...
        \item $\mathbf{w}_k=\mathbf{v}_k-\sum_{j=1}^{k-1}\frac{\langle \mathbf{v}_k, \mathbf{w}_j\rangle}{\norm{\mathbf{w}_j}^2}\mathbf{w}_j\,$ for $\,2\le k\le l.$
    \end{enumerate} 
    Then:
    \begin{enumerate}
...
Alternatively, we can derive the coefficients by calculus. We first show some results about matrix calculus.

\begin{definition}[Gradient]
Let $f: \mathbb{R}^n \to \mathbb{R}$ be a differentiable function. The gradient of $f$ is defined as:
$$ \nabla f = \frac{\partial f}{\partial \mathbf{x}} = \left( \frac{\partial f}{\partial x_1}, \frac{\partial f}{\partial x_2}, \dots, \frac{\partial f}{\partial x_n} \right)^\top.$$
\end{definition} 
...
Alternatively, define mappings $\phi$ and $\psi$ such that $f(\mathbf{x}) = (\psi \circ \phi)(\mathbf{x})$:
\begin{align*}
...
Recall our goal is to minimize the $\mathrm{SSE}(\beta)$.
Expand it to have:
\begin{align*}

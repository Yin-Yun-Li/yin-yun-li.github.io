...
These notes are based on two courses: \emph{Introduction to Linear Algebra I (Math4018)} \& \emph{Introduction to Linear Algebra II (Math4022)} taught by Prof. Kowk-Wing Tsoi at the Department of Mathematics, NTU. 
...
\begin{theorem}[Sufficient condition for similarity]
    ...
    \begin{enumerate}
        ...
        \item $m_A(x)=m_B(x):$ the same minimal polynomial.
    \end{enumerate}
...
\begin{enumerate}
    ...
    \item First prove the polynomial set satisfied by $A$ and the one satisfied by $B$ are equal. Then the monic polynomial of minimal degree satisfied by the corresponding matrix (i.e. minimal polynomial) is the same.
\end{enumerate}
...
\begin{pfbox}
    ...
    Apply $T$ on both sides and use the identity $T(\mathbf{v}_i)=\lambda_i\mathbf{v}_i\,\forall i=1,2,\cdots k$ (Equation 2).
...
\begin{exercise}
    Verify that 
    \begin{enumerate}
        \item $ker(T|_{W})=ker(T)\cap W$.
        \item $im(T|_{W})=\{T(\mathbf{w}):\mathbf{w}\in W\}.$
    \end{enumerate}
\end{exercise}
...
\input{bilinear_quadratic_definiteness.tex}

\newpage
\section{Markov Chain and Perron-Frobenius}
\input{MC_PF.tex}

\newpage
\section{Matrix Calculus}
\input{matrix_calculus.tex}
